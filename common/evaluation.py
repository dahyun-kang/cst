...
class AverageMeter:
    ...
    def update_seg(self, pred_mask, batch, loss=None):
        ...
        if loss:
            batchsize = float(pred_mask.shape[0])
            self.seg_loss_sum += loss * batchsize
            self.seg_loss_count += batchsize
    ...
    def return_class_mapping_dict(self, support_classes):
        ...
        batchsize = support_classes.shape[0]
        bg_classes = torch.zeros(batchsize, 1).to(support_classes.device).type(support_classes.dtype)
    ...
    def update_cls(self, pred_cls, gt_cls, loss=None):
        ...
        batchsize = float(pred_correct.shape[0])
        ''' accuracy '''
        # samplewise_acc = pred_correct.float().mean(dim=1)
        ''' exact ratio '''
        samplewise_er = torch.all(pred_correct, dim=1)
        self.cls_er_sum += samplewise_er.sum()
        self.cls_er_count += batchsize

        if loss:
            self.cls_loss_sum += loss * batchsize
            self.cls_loss_count += batchsize
