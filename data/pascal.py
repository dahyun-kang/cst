...
class DatasetPASCAL(Dataset):
    ...
    """
    def __init__(self, datapath, fold, transform, split, way, shot, transforms_aug=[]):
    ...
    def __getitem__(self, idx):
        ...
        if len(self.transforms_aug) > 0:
            query_img_augs = []
            for transform_aug in self.transforms_aug:
                out, ijhw = transform_aug(query_img)
                query_img_augs.append(out)
            query_img_augs = torch.cat(query_img_augs, dim=0) if len(query_img_augs) > 0 else []
        else:
            ijhw = []
            query_img_augs = []
        ...
        batch = {'query_img': query_img,
                 'query_img_augs': query_img_augs,
                 'query_img_augs_ijhw': ijhw,
